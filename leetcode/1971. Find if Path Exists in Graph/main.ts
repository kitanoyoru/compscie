...
const validPath = (_: number, edges: number[][], source: number, destination: number): boolean => {
    let graph: Graph = {};
...
const dfs = (graph: Graph, current: number, target: number, visited: Set<number>): boolean => {
    if (current == target) {
        return true;
    }
